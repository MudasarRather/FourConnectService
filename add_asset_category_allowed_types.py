"""Ad-hoc migration: add hr_asset_categories.allowed_asset_types (JSONB).
...
Run from the backend root:
    & "C:\\Users\\91700\\AppData\\Local\\Programs\\Python\\Python314\\python.exe" add_asset_category_allowed_types.py
"""
import os
import re
import sys
...
def main() -> int:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "ALTER TABLE hr_asset_categories "
                "ADD COLUMN IF NOT EXISTS allowed_asset_types JSONB"
            )
            cur.execute(
                "SELECT column_name, data_type FROM information_schema.columns "
                "WHERE table_name = 'hr_asset_categories' AND column_name = 'allowed_asset_types'"
            )
            row = cur.fetchone()
        if row:
            print(f"[OK] hr_asset_categories.allowed_asset_types present ({row[1]})")
            return 0
